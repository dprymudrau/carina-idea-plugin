...
public abstract class GenerateMembersHandlerBase implements CodeInsightActionHandler, ContextAwareActionHandler {
    private static final Logger LOG = Logger.getInstance(com.intellij.codeInsight.generation.GenerateMembersHandlerBase.class);
...
//        }
    ...
    protected ClassMember @Nullable [] chooseMembers(ClassMember[] members,
        ...
        MemberChooser<ClassMember> chooser = createMembersChooser(members, allowEmptySelection, false, project);
        if (editor != null) {
            final int offset = editor.getCaretModel().getOffset();
